...
export type FieldSelectProps = BaseSelectProps &
  (
    | (Pick<
...
/** Select 通过组件透传属性列表，其他属性可通过fieldProps属性进行透传 */
const transferPropsKeyList: (
  ...
  | 'labelInValue'
  | 'title'
)[] = ['options', 'onChange', 'value', 'labelInValue', 'title'];

/** 组件属性列表 */
export const propsKeyList: (keyof FieldSelectProps)[] = [
  /** ListItemStyle组件属性 */
...
const FieldSelect = (props: FieldSelectProps) => {
  ...
  const selectProps: SelectPickerProps = Object.assign(
    {},
    ...
    <>
      ...
      >
        <Text
          ...
          style={{
            textAlign: 'right',
            color: hasValue ? undefined : 'rgba(15, 26, 40, 0.45)',
          }}
        >
