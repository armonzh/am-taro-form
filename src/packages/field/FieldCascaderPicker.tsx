...
interface BaseCascaderPickerProps extends ListItemStyleProps {
  ...
  fieldProps?: Omit<CascaderPickerProps, 'value' | 'onChange' | 'labelInValue' | 'valueToList' | 'onConfirm' | 'onClose' | 'onCancel' | 'visible' | 'options'>;
  /** 提示文本 */
  placeholder?: string;
}

export type FieldCascaderPickerProps = BaseCascaderPickerProps & Pick<CascaderPickerProps, ArrayValueType<typeof transferPropsKeyList>>;

/** CascaderPicker 通过组件透传属性列表，其他属性可通过fieldProps属性进行透传 */
const transferPropsKeyList: ('options' | 'value' | 'labelInValue' | 'valueToList' | 'onChange')[] = ['options', 'value', 'labelInValue', 'valueToList', 'onChange'];

/** 组件属性列表 */
export const propsKeyList: (ArrayMergeType<keyof FieldCascaderPickerProps, typeof transferPropsKeyList> | keyof FieldCascaderPickerProps)[] = [
  /** ListItemStyle组件属性 */
...
const FieldCascaderPicker: React.FC<FieldCascaderPickerProps> = (props) => {
  const { placeholder = `请选择${props.label || ''}`, fieldProps = {}, value, onChange } = props;
  ...
  const valueStr = actionRef.current?.getSelectOptions?.(value)?.map((item: any) => item.label)?.join('');

  return (
    <>
      <ListItemStyle {...pick(props, ListItemStylePropsKeyList)} onClick={isEdit ? open : undefined}>
        <Text className="fx" style={{ textAlign: 'right', color: valueStr ? undefined : 'rgba(15, 26, 40, 0.45)' }}>
